...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(
      `INSERT INTO user 
      (first_name, last_name, birthday, email, password, role_id, hsc_number) 
      VALUES (?, ?, ?, ?, ?,  ?, ?)`,
      [first_name, last_name, birthday, email, hashedPassword, role_id, hsc_number]
    );
    const newUserId = result.insertId;
    const [resources] = await connection.query(`SELECT resource_id FROM resource`);

    if (role_id === PATIENT_ROLE_ID) {
      const patientPermissions = resources.map(r => [newUserId, newUserId, r.resource_id, 4]);
    ...
    res.status(201).json({ message: `User '${first_name} ${last_name}' created`, user_id: newUserId });
...
router.post('/login', async (req, res) => {
  ...
  try {
    const [users] = await db.query(`
      SELECT user.user_id, user.first_name, user.last_name, user.email, user.password, role.role_name AS role
      FROM user
