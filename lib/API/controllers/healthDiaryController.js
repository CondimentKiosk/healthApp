...
console.log('Incoming body:', req.body);
  ...
  try {
    ...
    const [diaryResult] = await conn.query(
      `INSERT INTO health_diary (entry_dateTime, entry_notes, entered_by, patient_id)
       VALUES ( ?, ?, ?, ?)`,
      [entry_dateTime, entry_notes, createdBy, patient_id]
    );
    ...
    for (const { symptom_name, rating } of symptoms) {
      let [rows] = await conn.query(
        `SELECT symptom_id 
         FROM symptom 
         WHERE symptom_name = ? 
           AND (is_predefined = 1 OR patient_id = ?) 
         LIMIT 1`,
        [symptom_name, patient_id]
      );
      ...
      if (rows.length > 0) {
        ...
        const [symptomResult] = await conn.query(
          `INSERT INTO symptom (symptom_name, is_predefined, patient_id, created_by_user_id)
           VALUES (?, 0, ?, ?)`,
          [symptom_name, patient_id, createdBy]
        );
...
const getHealthDiaryByPatient = async (req, res) => {
    ...
    try {
        const [rows] = await db.query(
            `SELECT hd.entry_id, hd.entry_dateTime, hd.entry_notes,
